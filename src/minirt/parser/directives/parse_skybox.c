...
typedef struct s_faces_pos {
	int	x;
	int	y;
}	t_faces_pos;

static t_error	_extract_faces_textures(t_object *obj);
static uint8_t	*_extract_face_pixels(
					int start[2],
...
static t_error	_extract_faces_textures(t_object *obj)
{
	...
	for (i = 0; i < NB_FACES; i++)
	{
		start[X] = faces[i].x;
		start[Y] = faces[i].y;
		pixels = _extract_face_pixels(start, face_height, face_width,face_size, obj->texture);
		if (pixels == NULL)
			return (FAILURE);
